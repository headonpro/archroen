...
def test_json_with_prose_prefix_is_parsed(monkeypatch, rule_csv):
    ...
    _, n_conf, _ = hx._rule_confirm_merge([], rule_csv, REPORT, {}, "r1")
    assert n_conf == 1


def test_json_without_results_key_warns(monkeypatch, rule_csv, capsys):
    _confirm(monkeypatch, '{"labels": []}')
    _, n_conf, n_cand = hx._rule_confirm_merge([], rule_csv, REPORT, {}, "r1")
    assert n_cand == 1 and n_conf == 0
...
def test_unparseable_output_warns_instead_of_silently_dropping(monkeypatch, rule_csv, capsys):
    ...
    assert "confirm" in out.lower() and "not usable" in out.lower()
